...
(async () => {
  ...
  let r = await eval_(`
    ...
    const existing = links.find(el => el.textContent.includes('existing account'));
    if (existing) {
    ...
    return 'not found';
  `);
  console.log("Existing account link:", r);
  ...
  // Should be at login page, click Continue with Google
  r = await eval_(`
    ...
    if (gTab) {
      ...
      if (r !== 'not found') {
        ...
        if (r !== 'not found') {
          ...
          console.log("Typed weber@bimopsstudio.com");
          await sleep(500);
