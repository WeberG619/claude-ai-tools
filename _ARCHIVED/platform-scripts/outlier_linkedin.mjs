...
(async () => {
  ...
  // Click "Connect LinkedIn" to see what it asks for
  let r = await eval_(`
    const btn = Array.from(document.querySelectorAll('button'))
      .find(b => b.textContent.includes('Connect LinkedIn'));
    if (btn) {
    ...
    return 'not found';
  `);
  console.log("LinkedIn button:", r);

  if (r !== 'not found') {
    const pos = JSON.parse(r);
    await clickAt(send, pos.x, pos.y);
    console.log("Clicked Connect LinkedIn");
    await sleep(2000);

    // Check what appeared - might be a dialog, input field, or OAuth redirect
    r = await eval_(`return document.body.innerText.substring(0, 5000)`);
    console.log("\nPage:", r);

    // Check for any new inputs, modals, dialogs
    r = await eval_(`
    ...
    r = await eval_(`
      ...
      return JSON.stringify(Array.from(modals).map(m => m.textContent.trim().substring(0, 200)));
    `);
    console.log("\nModals:", r);
  }
