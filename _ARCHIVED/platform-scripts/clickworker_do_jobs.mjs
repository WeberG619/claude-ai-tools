...
(async () => {
  ...
  r = await eval_(`
    ...
    return JSON.stringify(links.filter(a => {
      const href = a.href || '';
      return href.includes('/jobs/') && a.offsetParent !== null && a.textContent.trim().length > 3;
    }).map(a => ({
      text: a.textContent?.trim().substring(0, 80),
      href: a.href,
      parent: a.closest('tr, li, div.card, div.job, .list-group-item')?.textContent?.trim().substring(0, 120) || ''
    })));
  `);
  console.log("\nJob links:", r);
  ...
  fs.writeFileSync('D:\\_CLAUDE-TOOLS\\clickworker_jobs_list.png', Buffer.from(screenshot.data, 'base64'));
  console.log("\nScreenshot saved");
