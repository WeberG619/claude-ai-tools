...
(async () => {
  ...
  ws.addEventListener("open", () => {
    ...
    (async () => {
      // Page 1: Select Yes, enter ID, click Continue
      // Select "Yes" radio
      let r = await eval_(`
        (function() {
          var radio = document.getElementById('v_12x1');
          if (radio) {
            radio.checked = true;
      ...
      r = await eval_(`
        (function() {
          var input = document.getElementById('v_146');
          if (input) {
            var setter = Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype, 'value').set;
            setter.call(input, '25671709');
            input.dispatchEvent(new Event('input', { bubbles: true }));
            input.dispatchEvent(new Event('change', { bubbles: true }));
            return 'entered ID: ' + input.value;
          }
          return 'not found';
        })()
      `);
