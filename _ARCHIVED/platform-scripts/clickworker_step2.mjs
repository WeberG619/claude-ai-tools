...
(async () => {
  ...
  r = await eval_(`
    ...
    return JSON.stringify(Array.from(inputs).filter(i => i.offsetParent !== null).map(i => ({
      ...
      options: i.tagName === 'SELECT' ? Array.from(i.options).slice(0, 10).map(o => o.text.substring(0, 40)) : undefined
    })));
  `);
  console.log("\nForm fields:", r);
  ...
  r = await eval_(`
    ...
    return JSON.stringify(btns.filter(b => {
      ...
      return t.includes('continue') || t.includes('submit') || t.includes('next') || t.includes('save') || t.includes('confirm') || t.includes('finish');
    }).map(b => ({
      tag: b.tagName,
