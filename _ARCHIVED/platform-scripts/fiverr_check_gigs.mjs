...
(async () => {
  ...
  // Find a tab we can use - preferably not the clickworker one
  let tab = tabs.find(t => t.type === "page" && t.url.includes("fiverr"));
  if (!tab) {
    // Use the Gmail tab or any available tab
    tab = tabs.find(t => t.type === "page" && t.url.includes("mail.google"));
    if (!tab) tab = tabs.find(t => t.type === "page" && !t.url.includes("clickworker"));
  ...
  // Navigate to Fiverr seller dashboard / gigs page
