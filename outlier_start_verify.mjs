...
(async () => {
  ...
  let r = await eval_(`
    const btn = Array.from(document.querySelectorAll('button'))
      .find(b => b.textContent.includes('Start verification'));
    if (btn) {
    ...
    return 'not found';
  `);
  console.log("Start verification button:", r);

  if (r !== 'not found') {
    const pos = JSON.parse(r);
    await clickAt(send, pos.x, pos.y);
    console.log("Clicked Start verification!");
    await sleep(8000);
    ...
    // Check for any iframe (Persona often loads in iframe)
    r = await eval_(`
