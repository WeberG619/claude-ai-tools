...
# Fill email
$emailResult = Type-InField "#email" "weber@bimopsstudio.com"
Write-Output "Email field: $emailResult"

# Fill password
$passResult = Type-InField "#password" "B1m0ps!Resend2026"
Write-Output "Password field length: $($passResult.Length)"

# Click Create Account
Start-Sleep -Milliseconds 300
$r = Invoke-CDP "Runtime.evaluate" @{ expression = "var btns = Array.from(document.querySelectorAll('button')); var btn = btns.find(b => b.textContent.trim() === 'Create Account'); if(btn){btn.click();'clicked'}else{'not found'}" }
Write-Output "Button: $($r.result.result.value)"

# Wait for page change
Start-Sleep -Seconds 5
$r = Invoke-CDP "Runtime.evaluate" @{ expression = "document.title + ' | ' + window.location.href + ' | ' + document.body.innerText.substring(0,300)" }
Write-Output "After submit: $($r.result.result.value)"
