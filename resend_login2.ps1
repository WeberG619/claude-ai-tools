...
# Navigate to login
Invoke-CDP "Runtime.evaluate" @{ expression = "window.location.href='https://resend.com/login'" } | Out-Null
Start-Sleep -Seconds 4

# Fill fields
Type-InField "#email" "weberg619@gmail.com"
Type-InField "#password" "B1m0ps!Resend2026"

Start-Sleep -Milliseconds 300

# Find and click specifically the "Log In" button (not Google or GitHub)
$r = Invoke-CDP "Runtime.evaluate" @{ expression = "var btns = Array.from(document.querySelectorAll('button')); var logInBtn = btns.find(function(b) { return b.textContent.trim() === 'Log In'; }); if(logInBtn) { logInBtn.click(); 'clicked Log In button, type=' + logInBtn.type; } else { 'Buttons: ' + btns.map(function(b){return b.textContent.trim()}).join('|'); }" }
