...
def get_data_dir() -> Path:
    """Resolve data directory. Priority: env var > package-relative > XDG."""
    # 1. Explicit env var
    if os.environ.get("CLAUDE_MEMORY_DATA"):
        p = Path(os.environ["CLAUDE_MEMORY_DATA"])
        p.mkdir(parents=True, exist_ok=True)
        return p
    # 2. Package-relative (current/legacy layout)
    pkg_path = Path(__file__).parent.parent.parent / "data"
    if pkg_path.exists():
        return pkg_path
    # 3. XDG standard for new installs
    xdg = os.environ.get("XDG_DATA_HOME", str(Path.home() / ".local" / "share"))
    xdg_path = Path(xdg) / "claude-memory"
    xdg_path.mkdir(parents=True, exist_ok=True)
    return xdg_path

_SERVER_DIR = Path(__file__).parent.parent.parent
DB_PATH = get_data_dir() / "memories.db"
...
def get_engram():
    ...
    if _engram is None:
        try:
            from claude_memory.engram import EngramMemory, EngramConfig
            config = EngramConfig(
...
def memory_check_before_action(
    ...
    def _stem(word):
        """Naive suffix stripping for common English endings."""
    ...
    def _extract_words(text):
        """Split on whitespace, hyphens, and punctuation. Return stemmed terms."""
        tokens = _re.split(r'[\s\-_/.,;:!?()\[\]{}]+', text)
        return set(_stem(w) for w in tokens if len(w) > 3)

    for corr in corrections:
        content_lower = corr['content'].lower()

        # Extract key terms from correction (split on hyphens, stem words)
        corr_words = _extract_words(content_lower)
        action_words = _extract_words(combined)

        overlap = corr_words & action_words

        if len(overlap) >= 2:  # At least 2 significant stemmed words match
...
def main():
    """Entry point for the claude-memory CLI command."""
    mcp.run()
