...
Start-Sleep -Milliseconds 100
Invoke-CDP "Input.insertText" @{ text = "weberg619@gmail.com" } | Out-Null
Start-Sleep -Milliseconds 300

$r = Invoke-CDP "Runtime.evaluate" @{ expression = "document.querySelector('#email').value" }
Write-Output "Email: $($r.result.result.value)"

# Click Send Reset Instructions
$r = Invoke-CDP "Runtime.evaluate" @{ expression = "var b=Array.from(document.querySelectorAll('button')).find(function(x){return x.textContent.includes('Send Reset')});if(b){b.click();'clicked'}else{'not found: '+Array.from(document.querySelectorAll('button')).map(function(x){return x.textContent.trim()}).join(',')}" }
