...
(async () => {
  ...
  ws.addEventListener("open", () => {
    ...
    (async () => {
      ...
      let r = await eval_(`
        (function() {
          ...
          for (var i = 0; i < cbs.length; i++) {
            if (!cbs[i].checked) {
              cbs[i].click();
              return 'checked checkbox ' + i;
            }
          }
          if (cbs.length === 0) return 'no checkboxes found';
          return 'all already checked';
        })()
      `);
      L("Checkbox: " + r);
      await sleep(500);
      ...
      writeFileSync('D:\\_CLAUDE-TOOLS\\cw_uhrs_after_tos.png', Buffer.from(ss.data, 'base64'));
      L("\nScreenshot saved");
