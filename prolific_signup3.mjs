...
(async () => {
  ...
  let tab = tabs.find(t => t.type === "page" && (t.url.includes("prolific")));
  ...
  // 1. Accept cookies
  let r = await eval_(`
    const acceptBtn = Array.from(document.querySelectorAll('button')).find(b => b.textContent.trim() === 'Accept');
    if (acceptBtn) { acceptBtn.click(); return 'accepted'; }
    return 'not found';
  `);
  console.log("Cookies:", r);
  await sleep(500);

  // 2. Select "Take studies" radio
  r = await eval_(`
    const radios = document.querySelectorAll('input[type="radio"]');
    // Take studies is the first radio (participant)
    if (radios.length > 0) {
      radios[0].click();
      return 'selected first radio';
    }
    return 'no radios';
  `);
  console.log("Take studies:", r);
  await sleep(300);
