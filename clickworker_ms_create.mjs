...
(async () => {
  ...
  let r = await eval_(`
    ...
    const createLink = links.find(a => a.textContent.toLowerCase().includes('create'));
    if (createLink) { createLink.click(); return 'clicked: ' + createLink.href; }
    return 'not found - links: ' + links.map(a => a.textContent.trim().substring(0, 40)).join(' | ');
  `);
  console.log("Create account:", r);
  await sleep(5000);
  ...
  fs.writeFileSync('D:\\_CLAUDE-TOOLS\\clickworker_ms_create.png', Buffer.from(screenshot.data, 'base64'));
  console.log("\nScreenshot saved");
