...
(async () => {
  ...
  ws.addEventListener("open", () => {
    ...
    (async () => {
      ...
      // If error page, click Refresh
      if (pageText.toLowerCase().includes('oops') || pageText.toLowerCase().includes('went wrong')) {
        L("Error page detected, clicking Refresh...");
        let r = await clickButton('refresh');
        L("Refresh: " + r);
        await sleep(5000);
        pageText = await eval_(`document.body.innerText.substring(0, 1000)`);
        L("After refresh: " + pageText.substring(0, 300));
      }

      // Now continue with profile questions
      let lastQ = '';
      let stuckCount = 0;
      ...
      for (let step = 0; step < 20; step++) {
        ...
        // Completion check
        if (ql.includes('profile complete') || ql.includes('all done') || (ql.includes('survey') && ql.includes('available')) || ql.includes('no surveys') || ql.includes('earn rewards')) {
        ...
        // Employment
        else if (ql.includes('employ') && !ql.includes('how many employ')) {
        ...
        // State
        else if (ql.includes('state') && !ql.includes('united states') && !ql.includes('employment status')) {
        ...
        else if (ql.includes('purchasing') || ql.includes('buying decision')) {
          r = await clickRadio('Yes');
          L("  Purchasing: " + r); answered = true;
        }
        // Number of people in household
        else if (ql.includes('household') && (ql.includes('how many') || ql.includes('number'))) {
          r = await clickRadio('1');
          if (r.includes('not found')) r = await setInput('1');
          L("  Household size: " + r); answered = true;
        }
        // Region
