...
(async () => {
  ...
  // Click Yes
  let r = await eval_(`
    const btn = Array.from(document.querySelectorAll('button[type="submit"]')).find(b => b.textContent.trim() === 'Yes');
    if (btn) { btn.click(); return 'clicked Yes'; }
    return 'Yes not found';
  `);
  console.log("Stay signed in:", r);
  await sleep(8000);
  ...
  // Check for consent/permission grant page
  r = await eval_(`
  ...
  fs.writeFileSync('D:\\_CLAUDE-TOOLS\\clickworker_ms_yes.png', Buffer.from(screenshot.data, 'base64'));
  console.log("\nScreenshot saved");
